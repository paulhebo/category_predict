...
class PCDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        i2cat = sorted(
            k for k, v in cat2cnt.items() if v > 1
        )
        ...
        self.train_dataset = mk_ds(
            list(self.df_trn.keyword), self.tokenizer, self.max_seq_length, self.ys_trn
        )
        self.eval_dataset = mk_ds(
            list(self.df_val.keyword), self.tokenizer, self.max_seq_length, self.ys_val
        )
